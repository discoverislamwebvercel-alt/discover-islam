...
const schema = z
  ...
  .refine(
    data => {
      // If "other" is selected, otherTopic must be provided
      if (data.topics === 'other') {
        return data.otherTopic && data.otherTopic.trim().length > 0;
      }
      return true;
    },
    {
...
const SchoolVisitForm = ({
  ...
  // Watch the topics field to show/hide the "Other" textarea
  const selectedTopic = useWatch({
    control,
    name: 'topics',
  });

  // Clear otherTopic when switching away from "other"
  useEffect(() => {
    ...
    <section className='relative w-full bg-[#FFFFFF]'>
      <div className='mx-auto flex max-w-[1230px] flex-col items-center px-4 py-16 sm:py-20 md:py-24 lg:py-28'>
        ...
        <motion.h1
          ...
          className='text-center font-extrabold text-[#408360] tracking-[-0.03em] leading-[0.94] text-4xl sm:text-5xl md:text-6xl lg:text-[80px]'
        >
          In School Visit
        </motion.h1>
        ...
        {!buttonHidden && (
          ...
          >
            <Button
              variant='primary'
              className='!w-[280px] !h-[50px] !px-[25px] !py-[12px] !rounded-[40px] !text-[18px] sm:!w-[320px] sm:!h-[55px] sm:!px-[30px] sm:!py-[14px] sm:!text-[20px] md:!w-[360px] md:!h-[60px] md:!px-[35px] md:!py-[16px] md:!text-[22px] lg:!w-[404px] lg:!h-[67px] lg:!px-[37px] lg:!py-[18px] lg:!rounded-[52px] lg:!text-[26px] !font-extrabold !leading-[1] tracking-[0] shadow-sm'
              onClick={async () => {
                setIsHidden('block');
      ...
      >
        ...
        >
          ...
          {/* Animated "Other" topic textarea - spans full width */}
          {selectedTopic === 'other' && (
            <motion.div
